...
// flag: new game = -1, input mode = 0, flag mode = 1, check mode = 2, end game = 3
int game_mode=-1;
...
void flag_mode(int* nMines){
	...
	do {
		...
		if(direction == '8') {
			// up
			cursor_y = (MAX + cursor_y -1) % MAX;
		} else if(direction == '2') {
			// down
			cursor_y = (1+cursor_y) % MAX;
		} else if(direction == '4') {
			cursor_x = (MAX + cursor_x-1) % MAX;
		} else if(direction == '6') {
			cursor_x = (1+cursor_x) % MAX;
		} else if(direction == 'c' || direction == 'C') {
			game_mode = 2;
			return;
		} else if(direction == '\n') {
			value = table_array[cursor_y][cursor_x];

			if (value == 99) {				// mine case
				table_array[cursor_y][cursor_x] += 1;
				*nMines -= 1;				// mine found
			} else if(value >= 0 && value <= 8) {	// number of mines case (the next cell is a mine)
				table_array[cursor_y][cursor_x] += 20;
...
void new_game(int* nMines, int argc, char *argv[]){
...
int main(int argc, char *argv[]) {
	...
	//
	do{			// when nMines becomes 0 you will win the game
		if(game_mode == -1)
			new_game(p_nMines, argc, argv);
		...
		if(game_mode == 1)
			...
			check_mode();
		if(game_mode == 3){
			end_of_game(p_nMines);
		}
	} while(nMines != 0 && game_mode != 3);
